...
import TruthCheckPanel from "@/components/TruthCheckPanel";
...
interface ResearchWorkspaceProps {
  ...
  initialTab?: "research" | "truthcheck";
}
...
export default function ResearchWorkspace({ 
  ...
  const [activeTab, setActiveTab] = useState<"research" | "truthcheck">("research");
  ...
  // Update active tab on initialization/change
  useEffect(() => {
    if (initialTab) {
  ...
  useEffect(() => {
    if (prefilledQuery) {
      ...
      if (prefilledMode) {
        setFilters(prev => ({ ...prev, depth: prefilledMode }));
      }
      handleSearch(prefilledQuery);
    }
  }, [prefilledQuery, prefilledMode]);
  ...
  return (
    <div className={`grid ${activeTab === "research" ? "grid-cols-1 xl:grid-cols-[280px_1fr_320px]" : "grid-cols-1"} gap-6 min-h-[calc(100vh-48px)]`}>
      ...
      <main className="flex flex-col gap-6 min-h-[calc(100vh-48px)]">
        ...
        <section className="glass-panel p-8 relative overflow-hidden bg-black/30">
          ...
          <div className="flex flex-col gap-4">
            ...
            {/* Tab Switcher */}
            <div className="flex border-b border-white/5 pb-2 font-mono text-xs gap-4 mt-2">
              <button
                type="button"
                onClick={() => setActiveTab("research")}
                className={`pb-2 px-1 relative transition-all duration-200 cursor-pointer select-none ${
                  activeTab === "research" ? "text-cyber-purple font-extrabold" : "text-white/40 hover:text-white"
                }`}
              >
                Research Workspace
                {activeTab === "research" && (
                  <motion.div layoutId="activeTabUnderline" className="absolute bottom-0 left-0 right-0 h-[2px] bg-cyber-purple" />
                )}
              </button>
              <button
                type="button"
                onClick={() => setActiveTab("truthcheck")}
                className={`pb-2 px-1 relative transition-all duration-200 cursor-pointer select-none ${
                  activeTab === "truthcheck" ? "text-cyber-pink font-extrabold" : "text-white/40 hover:text-white"
                }`}
              >
                TruthCheck AI
                {activeTab === "truthcheck" && (
                  <motion.div layoutId="activeTabUnderline" className="absolute bottom-0 left-0 right-0 h-[2px] bg-cyber-pink" />
                )}
              </button>
            </div>
            ...
            {activeTab === "truthcheck" && (
              <div className="space-y-1">
                <h1 className="text-3xl font-extrabold tracking-tight text-white leading-tight">
                  TruthCheck AI Workspace
                </h1>
                <p className="text-white/60 text-xs font-sans max-w-2xl">
                  Analyze claims, cross-examine sources, detect contradictions, and review verified consensus reports.
                </p>
              </div>
            )}
